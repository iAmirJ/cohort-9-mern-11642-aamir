  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        ...
        setError('Failed to fetch trash.');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="text-slate-500 mt-10">Your trash is empty.</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {notes.map((note) => (
              <div key={note.id} className="relative group flex flex-col justify-between p-5 rounded-lg border border-gray-200 bg-surface">
                <div>
                  ...
                  <p className="text-slate-400 text-sm line-clamp-3 mb-4 leading-relaxed">
                    {typeof note.content === 'string' ? note.content.replace(/<[^>]*>?/gm, '').substring(0, 100) : 'Content preview not available.'}
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleRestore(note.id)} className="p-1.5 text-primary hover:bg-primary/10 rounded" title="Restore">
                      <RotateCcw className="w-4 h-4" />
                    </button>
                    <button onClick={() => handlePermanentDelete(note.id)} className="p-1.5 text-error hover:bg-error/10 rounded" title="Permanent Delete">
