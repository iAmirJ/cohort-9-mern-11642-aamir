...
describe('socket (before initSocket has ever run)', () => {
  it('broadcastToUser is a safe no-op', () => {
    expect(() => broadcastToUser('u1', 'note:created', { id: 'n1' })).to.not.throw();
  });
});
...
describe('socket (after initSocket)', () => {
  ...
  it('rejects a connection with an invalid/garbage token', (done) => {
    const client = ioClient(url, { auth: { token: 'garbage.invalid.token' }, reconnection: false });
    client.on('connect_error', (err) => {
  ...
  it('accepts a connection with a valid token via the auth object', (done) => {
    const client = ioClient(url, { auth: { token: signToken('u1') }, reconnection: false });
    client.on('connect', () => {
      client.close();
      done();
    });
  ...
  it('accepts a connection with a valid token via a query param (Postman-compatible)', (done) => {
    const client = ioClient(url, { query: { token: signToken('u1') }, reconnection: false });
    client.on('connect', () => {
      client.close();
      done();
    });
  ...
  it('delivers a broadcast only to sockets belonging to that user', (done) => {
    const clientA = ioClient(url, { auth: { token: signToken('user-A') }, reconnection: false });
    const clientB = ioClient(url, { auth: { token: signToken('user-B') }, reconnection: false });
  ...
  it('a user with two open tabs (two sockets) both receive the same broadcast', (done) => {
    const tabOne = ioClient(url, { auth: { token: signToken('u1') }, reconnection: false });
    const tabTwo = ioClient(url, { auth: { token: signToken('u1') }, reconnection: false });

    let tabOneGotIt = false;
    let tabTwoGotIt = false;

    tabOne.on('note:deleted', () => { tabOneGotIt = true; });
    tabTwo.on('note:deleted', () => { tabTwoGotIt = true; });
